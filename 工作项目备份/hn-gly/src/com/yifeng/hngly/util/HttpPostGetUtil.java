...
public class HttpPostGetUtil {
	...
	public static String doGetLogin(String url, Map params) {
		...
		try {
			...
			if (httpResponse.getStatusLine().getStatusCode() == 200) {
				...
				strResult = SERVICEERROR;
			}
		} catch (ClientProtocolException e) {
			strResult = ERROR;
		} catch (SocketTimeoutException e){
			strResult=TIMEOUTERROR;
		}catch (ConnectTimeoutException  e) {
			strResult=SERVERNOTFOUND;
		}
	...
	// private static void saveCookieToDB(String cookies) {
	// SharedPreferences rmd =AppContext.get(). getSharedPreferences("COOKIE",
	// 0);
	// rmd.edit().putString("cookies", cookies)
	// .commit();
	// }
	//
	// private void getCookieFromDB(){
	// SharedPreferences user_n_p =AppContext.get().
	// getSharedPreferences("COOKIE", 0);
	// String cookies = user_n_p.getString("cookies", "");
	// String[] aCookie=cookies.split("||");
	// }

...
public class DataConvert {
	 ...
	 * @return map
	 */
	public static Map<String, String> toMap1(String json) {
		...
		if (json.equals("")) {
			...
			try {
				LinkedHashMap<String, String> jdata = paraMap(json);
				if (jdata.size() == 0) {
					map.put("state", String.valueOf(ConstantUtil.IS_EMPTY));
				} else {
					map.put("state", String.valueOf(ConstantUtil.LOGIN_SUCCESS));
					map.putAll(jdata);
					// for (Iterator iterator = jdata.keys();
					// iterator.hasNext();) {
					// String key = (String)iterator.next();
					...
					// }else
					// if(jdata.get(key)==null||jdata.get(key).toString().equals("null")){
					// map.put(key,"");
					// }
					// else{
	 ...
	 * @return
	 */
	public static List<Map<String, String>> toConvertStringList(String json,
			String keyName) {
		List<Map<String, String>> list = new ArrayList<Map<String, String>>();
		...
		try {
			...
			if (map.get("state").equals(
				...
				if (map.get("success").equals("true")) {
					List<Map<String, String>> nlist = toArrayList(map
							.get(keyName));
	 ...
	 * @return
	 */
	public static List<Map<String, Object>> toConvertObjectList(String json,
			String keyName) {
		...
		try {
			...
			if (map.get("state").equals(
					String.valueOf(ConstantUtil.LOGIN_SUCCESS))) {
				if (map.get("success").equals("true")) {
					List<Map<String, Object>> nlist = toObjectArrayList((String) map
							.get(keyName));
					for (int i = 0; i < nlist.size(); i++) {
	 ...
	 * @return
	 */
	public static Map<String, String> toConvertStringMap(String json,
			String keyName) {
		Map<String, String> map = null;
		try {
			map = toMap(json);
	 ...
	 * @return
	 */
	public static Map<String, Object> toConvertObjectMap(String json,
			String keyName) {
		Map<String, Object> map = null;
		try {
			map = toObjectMap(json);
	...
	private static LinkedHashMap<String, String> paraMap(String json) {
		...
		// if(ts[0].startsWith("\""))
		// ts[0]= ts[0].substring(1, ts[0].length()-1);
		// if(ts[1].startsWith("\""))
		// ts[1]=ts[1].substring(1, ts[1].length()-1);
		// map.put(String.valueOf(ts[0].trim()), String.valueOf(ts[1].trim()));
		// }
		// return map;
		...
		for (int i = 0; i < length; i++) {
			...
			if (ifcheck) {
				...
				if (tc == ',') {
					...
					if (ifadd) {
						ifadd = false;
						map.put(str1.toString().replaceAll("\"", ""),
								str2.toString());
						str1.setLength(0);
	 ...
	 * @return
	 */
	public static List<Map<String, String>> toAreaList(String array) {
		List<Map<String, String>> list = new ArrayList<Map<String, String>>();
		Map<String, String> map = new HashMap<String, String>();
		map.put("id", "");
		map.put("name", "全部");

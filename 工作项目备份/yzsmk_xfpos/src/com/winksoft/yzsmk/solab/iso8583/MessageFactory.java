...
*/
package com.winksoft.yzsmk.solab.iso8583;
...
import com.winksoft.yzsmk.solab.iso8583.parse.FieldParseInfo;
...
public class MessageFactory<T extends IsoMessage> {
	...
	private boolean useCompress; //ѹ��BCD
	private int etx = -1;
	...
	throws ParseException, UnsupportedEncodingException {
		isoHeaderLength = 0;	// zhuyin 2016-10-10  去除 TPDU
		...
		if (index == null) {
			Log.e(tag, String.format("ISO8583 MessageFactory has no parsing guide for message type %04x [%s]",
				type, new String(buf)));
			return null;
		}
		...
		for (int i = 1; i < bs.length(); i++) {
			if (bs.get(i) && !index.contains(i+1)) {
				Log.w(tag, String.format("ISO8583 MessageFactory cannot parse field %d: unspecified in parsing guide", i+1));
				abandon = true;
			}
		}
		if (abandon) {
			...
			for (Integer i : index) {
				...
				if (bs.get(i - 1)) {
					if (ignoreLast && pos >= buf.length && i.intValue() == index.get(index.size() -1)) {
						Log.w(tag, String.format("Field %d is not really in the message even though it's in the bitmap", i));
						bs.clear(i - 1);
					} else {
						...
						if (val != null) {
							if (val.getType() == IsoType.NUMERIC || val.getType() == IsoType.DATE10
									|| val.getType() == IsoType.DATE4 ||  val.getType() == IsoType.DATE8 ||val.getType() == IsoType.DATE_EXP
									|| val.getType() == IsoType.AMOUNT || val.getType() == IsoType.TIME || 
									val.getType() == IsoType.LLBCD || val.getType() == IsoType.LLLBCD) {
							...
							if (val.getType() == IsoType.LLVAR || val.getType() == IsoType.LLBIN || val.getType() == IsoType.LLBCD) {
								pos++;
							} else if (val.getType() == IsoType.LLLVAR || val.getType() == IsoType.LLLBIN || val.getType() == IsoType.LLLBCD) {
								pos += 2;
							}
						}
					}
	...
	public void setParseMap(int type, Map<Integer, FieldParseInfo> map) {
		...
		Log.v(tag, String.format("ISO8583 MessageFactory adding parse map for type %04x with fields %s", type, index));

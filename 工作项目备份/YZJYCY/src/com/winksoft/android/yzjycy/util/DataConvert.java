...
public class DataConvert {
	...
	public static List<Map<String, String>> toArrayList(String array) {
		...
		if (array.equals("")) {
			map.put("state", String.valueOf(Constants.IS_EMPTY));
			list.add(map);
			return list;
		} else if (StringHelper.eregi_replace("(\r\n|\r|\n|\n\r)", "", array)
				.equals("error")) {
			map.put("state", String.valueOf(Constants.SERVER_ERROR));
			list.add(map);
			return list;
		} else if (array.equals("FAIL")) {// key失败
			map.put("state", String.valueOf(Constants.KEY_ERROR));
			list.add(map);
			return list;
		} else {
			try {
				...
				if (jdata.length() == 0) {
					...
					for (int i = 0; i < jdata.length(); i++) {
						...
						map1.put("state",
								String.valueOf(Constants.LOGIN_SUCCESS));
				...
				map.put("state", String.valueOf(Constants.INNER_ERROR));
				list.add(map);
			}
			return list;
		}
		...
		if (json.equals("")) {
			...
			try {
				...
				if (jdata.length() == 0) {
					...
					map.put("state", String.valueOf(Constants.LOGIN_SUCCESS));
		...
		try {
			...
			if (map.get("state").equals(
				...
				if (map.get("success").equals("true")) {
					...
					map.put("state", String.valueOf(Constants.LOGIN_FAIL));
					list.add(map);
				}
			} else {
				list.add(map);

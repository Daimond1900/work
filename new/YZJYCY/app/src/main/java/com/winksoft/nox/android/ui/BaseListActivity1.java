 ...
 */
public abstract class BaseListActivity1 extends Fragment {
	 ...
	 *            是否下拉刷新
	 */
	protected void intiListview(boolean load, boolean showHead , View view) {
		this.listview = (YFListView) view.findViewById(R.id.listview);
		listview.load = load;
		listview.showHead = showHead;
		listview.init(getActivity());

		if (footbar == null) {
			footbar = new ListViewUtil(getActivity(), listview, getResources());
		}

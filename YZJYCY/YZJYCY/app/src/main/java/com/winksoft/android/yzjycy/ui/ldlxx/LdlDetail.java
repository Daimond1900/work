...
public class LdlDetail extends BaseActivity {
	...
	private String sfzStr,nameStr,lxdhStr,mzStr,zzmmStr,hkxzStr,whcdStr,jtzzStr;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mzStr = mz.getText().toString();
		zzmmStr = zzmm.getText().toString();
		hkxzStr = hkxz.getText().toString();
		whcdStr = whcd.getText().toString();
		jtzzStr = jtzz.getText().toString();
		initAllEdittext();
	}
	...
	private void initView() {
		...
		new AsyncTask<Void, Void, Map<String, String>>() {
			...
			protected void onPreExecute() {
				super.onPreExecute();
				progressDialog = LdlDetail.this.commonUtil.showProgressDialog("数据加载中...");
				...
				if (cansave)
					new AsyncTask<Void, Void, Boolean>() {
						...
						protected Boolean doInBackground(Void... params) {
							...
							// jyzt;
							return dal
									.modifyLabourInfo(sfzStr, nameStr,
											lxdhStr,
											ParseData.getKeyByValue(ParseData.MAPMZ, mzStr),
											ParseData.getKeyByValue(ParseData.ZZMM, zzmmStr),
											ParseData.getKeyByValue(ParseData.HKZT, hkxzStr),
											ParseData.getKeyByValue(ParseData.WHCD, whcdStr),
											jtzzStr, user.getUserId(), "", // user.getOperatorname()
											user.getOrgid(), "", // user.getZzs051()
											"", "")// 1.user.getAaa020() 2.
													// user.getAaa021() /*需要修改*/
									.isSuccess();
						...
						protected void onPreExecute() {
							...
							progressDialog = LdlDetail.this.commonUtil.showProgressDialog(save.getText() + "基本信息中...");
	...
	private String getAllString() {
		...
		return sfz.getText().toString() + "," + name.getText().toString() + "," + xb.getText().toString() + ","
				+ csrq.getText().toString() + "," + lxdh.getText().toString() + "," + whcd.getText().toString() + ","
				+ hkxz.getText().toString() + "," + mz.getText().toString() + "," + zzmm.getText().toString() + ","
				+ jtzz.getText().toString() + "," + jyzt.getText().toString();
	}

	private void setButtonClisk(final EditText b, final String[] whcdvalue, final String title) {
		b.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				new AlertDialog.Builder(LdlDetail.this).setTitle(title)
						.setItems(whcdvalue, new DialogInterface.OnClickListener() {

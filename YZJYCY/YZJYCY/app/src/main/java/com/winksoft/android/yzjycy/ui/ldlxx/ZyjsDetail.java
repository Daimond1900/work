...
public class ZyjsDetail extends BaseActivity {
	...
	private String sfzStr,bzsmStr,jsyyStr;
	@Override
...
//,,
		...
		bzsmStr = bzsm.getText().toString();
		jsyyStr = jsyy.getText().toString();
		...
		save.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				new AsyncTask<Void, Void, Boolean>() {
					// sfzStr,bzsmStr,jsyyStr
					@Override
					protected Boolean doInBackground(Void... params) {
						ReJson json = dal.modifyResourceReduction(sfzStr,
								ParseData.getKeyByValue(ParseData.JSYY, jsyyStr),
								bzsmStr, ZyjsDetail.this.user.getUserId(), "", // ZyjsDetail.this.user.getOperatorname()
																									// 需要修改
								ZyjsDetail.this.user.getOrgid(), ""); // ZyjsDetail.this.user.getZzs051()
	...
	private void initAllTextView() {
		// setButtonClisk(jsyy, ParseData.getMapAllValue(ParseData.JSYY),
		// "选择减少原因");
	}

	private String getAllString() {
		return sfz.getText().toString() + "," + name.getText().toString() + "," + jsyy.getText().toString() + ","
				+ bzsm.getText().toString();
	}
	...
	private void setButtonClisk(final TextView b, final String[] whcdvalue, final String title) {
		b.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				new AlertDialog.Builder(ZyjsDetail.this).setTitle(title)

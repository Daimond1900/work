...
public class QzyyDetail extends BaseActivity {
	...
	Button save, back;
	private String sfzStr, qzgzStr, qzdqStr,yxgzjwStr;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		qzgzStr = qzgz.getText().toString();
		qzdqStr = qzdq.getText().toString();
		yxgzjwStr = yxgzjw.getText().toString();
			...
			public void onClick(View v) {
				new AsyncTask<Void, Void, Boolean>() {
					...
					protected Boolean doInBackground(Void... params) {
						ReJson json = dal.modifyJobAspiration(sfzStr, qzgzStr,
								ParseData.getKeyByValue(ParseData.JYDQXS, qzdqStr),
								yxgzjwStr, QzyyDetail.this.user.getUserId(), "", // QzyyDetail.this.user.getOperatorname()
																									// 需要修改
								QzyyDetail.this.user.getOrgid(), ""); // QzyyDetail.this.user.getZzs051()
	...
	private String getAllString() {
		return sfz.getText().toString() + "," + name.getText().toString() + "," + qzgz.getText().toString() + ","
				+ qzdq.getText().toString() + "," + yxgzjw.getText().toString() + "," + djsj.getText().toString();
	...
	private void setButtonClisk(final TextView b, final String[] whcdvalue, final String title) {
		b.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				new AlertDialog.Builder(QzyyDetail.this).setTitle(title)

...
public class CpuJBD {
	...
	public final static byte PSAM_INS_READ_RECORD   = (byte)0xB2;  //读取记录文件
	...
	public final static byte PSAM_CIRCLE 			= (byte)0x52;  //圈存
	...
	public final static byte PSAM_P1_DEBIT         = (byte)0x01;  //消费初始化
	public final static byte PSAM_P2_SF_CURR        = (byte)0x00;  //选择当前文件
	public final static byte PSAM_P2_SF_NEXT        = (byte)0x02;  //选择下一个文件
	public final static byte PSAM_INS_BALANCE       = (byte)0x5C;  //读取余额
	public final static byte PSAM_INS_DEBIT_INIT    = (byte)0x50;  //消费初始化
	public final static byte PSAM_INS_DEBIT         = (byte)0x54;  //消费初始化
	...
	public byte[] getValidReturn(byte[] recvBuf) throws CpuException
	{
		if ((recvBuf == null) || (recvBuf.length<2) ||(!((recvBuf[recvBuf.length-2] == (byte)0x90) || (recvBuf[recvBuf.length-2] == (byte)0x61))))
		{
			throw new CpuException(0, "返回值错误！");
	...
	public void selectFileByName(String FileName) throws CpuException
	{
		byte[] m = null;
		try {
			m = FileName.getBytes("GB2312");
		} catch (UnsupportedEncodingException e) {
			throw new CpuException(1, "文件名转换错误！");
		}
	...
	public byte[] selectFileById(byte[] id) throws CpuException
	{
	...
	public byte[] selectFileByAID(byte[] id) throws CpuException
	{
		ByteBuffer buff = ByteBuffer.allocate(5+id.length);
		buff.put(PSAM_CLA_NORMAL)
			.put(PSAM_INS_SELECT_FILE)
	...
	//FileLen: 读取数据长度
	//data: 返回的数据
	public byte[] getRecordFile(int recordNumber, byte SFI) throws CpuException
	{
		ByteBuffer buff = ByteBuffer.allocate(5);
		buff.put(PSAM_CLA_NORMAL)
			.put(PSAM_INS_READ_RECORD)
			.put((byte)(recordNumber&0xff))	// P1
			.put((byte) (((SFI<<3)&0xF8)|0x4))  // P2
			.put((byte)0x00);   // LC 不存在
	...
	//FileLen: 读取数据长度
	//data: 返回的数据
	public byte[] getBinaryFile(boolean byShort, byte SFI, int offset, byte fileLen) throws CpuException
	{
		ByteBuffer buff = ByteBuffer.allocate(5);
		buff.put(PSAM_CLA_NORMAL)
	...
	//取随机数
	public byte[] GetChallenge() throws CpuException
	{
	...
	public byte[] getResponse(byte readLen) throws CpuException
	{
		ByteBuffer buff = ByteBuffer.allocate(5);
		buff.put(PSAM_CLA_NORMAL)
	...
	public byte[] iniForPurchase(byte[] MacData) throws CpuException
	{
	...
	public byte[] creditForPurchase(byte[] Mac) throws CpuException
	{
	...
	public byte[] deliverySessionKey(byte[] sDe, byte keyID)throws CpuException
	{
		if (sDe.length != 16)
			throw new CpuException(0, "sDelivery Session error");
	...
	public byte[] deliveryKey(byte[] sDe, byte keyType, byte keyID)throws CpuException
	{
		if (sDe.length != 8)
			throw new CpuException(0, "sDelivery error");
		ByteBuffer buff = ByteBuffer.allocate(5+sDe.length);
		buff.put(PSAM_CLA_SPEC)
			.put(PSAM_INS_DK)
			.put(keyType)
	...
	public byte[] cipher(byte[] ciper, byte P1, byte P2)throws CpuException
	{
		if (ciper.length %8 != 0)
			throw new CpuException(0, "sCiper error");
	...
	public byte[] calculateKey(byte keyID, byte[] data)throws CpuException
	{
	...
	public int getBalance() throws CpuException
	{
		byte[] id = new byte[4];
		ByteBuffer buff = ByteBuffer.allocate(5+id.length);
		buff.put(PSAM_CLA_SPEC)
			.put(PSAM_INS_BALANCE)
			.put(PSAM_P1_SF_BY_ID)
			.put(PSAM_P2_SF_NEXT)
			.put((byte)id.length)
			.put(id);
		...
		byte[] recvBuf = transmit(buff.array());
		byte[] tmp = getValidReturn(recvBuf);
		return ((tmp[0] & 0XFF) << 24) | ((tmp[1] & 0XFF) << 16)
		| ((tmp[2] & 0XFF) << 8) | (tmp[3] & 0XFF);
	}
	
	public byte[] debitInit(byte[] intiData,byte debitFlag) throws CpuException
	{
		ByteBuffer buff = ByteBuffer.allocate(5+intiData.length);
		buff.put(PSAM_CLA_SPEC)
			.put(PSAM_INS_DEBIT_INIT)
			.put(debitFlag)
			.put(PSAM_P2_SF_NEXT)
			.put((byte)intiData.length)
			.put(intiData);
	...
	public byte[] circleDeposit(byte[] circleData) throws CpuException
	{
		ByteBuffer buff = ByteBuffer.allocate(5+circleData.length);
		buff.put(PSAM_CLA_SPEC)
			.put(PSAM_CIRCLE)
			.put((byte)0x00)
			.put((byte)0x00)
			.put((byte)circleData.length)
			.put(circleData);
	...
	public byte[] deductions(byte[] deductionsBuf) throws CpuException
	{
		ByteBuffer buff = ByteBuffer.allocate(5+deductionsBuf.length);
		buff.put(PSAM_CLA_SPEC)
			.put(PSAM_INS_DEBIT)
			.put(PSAM_P1_DEBIT)
			.put(PSAM_P2_SF_CURR)
			.put((byte)deductionsBuf.length)
			.put(deductionsBuf);

...
public class BusinessMode {
	 ...
	 * 本地路径.
	 */
	public static String LocalPath_ORI =  Environment.getExternalStorageDirectory() + "/CZ/filePath_ORI/"; //消费业务为:/XF/,充值业务为:/CZ/
	public static String LocalPath_SUCCESS =  Environment.getExternalStorageDirectory() + "/CZ/filePath_SUCCESS/";
	public static String LocalPath_FAIL =  Environment.getExternalStorageDirectory() + "/CZ/filePath_FAIL/";
	public FTP ftp;
	...
	public boolean manualOp()
	{
		...
		if(FilesUtil.hasUploadFiles(LocalPath_ORI)>0)
		{
		  ...
		  if(ret)
		  {
			saveLog("文件手动上传成功","0"); //9：正常FTP上传消费数据，0：正常FTP上传充值数据
		  }
		  else
		  {
			saveLog("文件手动上传失败","6"); //5：FTP上传消费数据异常，6：FTP上传充值数据异常
		  }
		}
		else
		{
	...
	public void buildFile()
	{
		  ...
		  )
		{
			...
			if(FilesUtil.hasUploadFiles(LocalPath_ORI)>0)
			{
			  ...
			  if(ret)
			  {
				saveLog("文件上传成功","0"); //9：正常FTP上传消费数据，0：正常FTP上传充值数据
			  }
			  else
			  {
				saveLog("文件上传失败","6"); //5：FTP上传消费数据异常，6：FTP上传充值数据异常
			  }
			}
			else
			{
	...
	public long saveToxfDb_desfire(Map<String, String> map,Context context)
	{
		...
		try
		{
			...
			ret = sqlInstance.insert("desfire",map);
		}
		catch(Exception e)
		{
			Log.d(tag,"saveToxfDb err:" + e.getMessage() + "," + e.getStackTrace());

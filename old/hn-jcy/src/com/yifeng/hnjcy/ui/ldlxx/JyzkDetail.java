...
import com.yifeng.hnjcy.util.ActivityStackControlUtil;
...
public class JyzkDetail extends BaseActivity {
	private TextView sfz, name, jbqc, jyzt, mqjycy, cyhy, jyxs, sffxcyry,
			jydqlx, jydq, jyfs, wgnx, jnwgsj, wgygz, sfqdldht, sfjgbm,
			swzyryjyyx;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.ldl_jyzk_detail);
				...
				if (cansave)
					new AsyncTask<Void, Void, Boolean>() {
						...
						protected Boolean doInBackground(Void... params) {
							...
							// sfqdldht, sfjgbm,
							// swzyryjyyx
							ReJson json =dal.modifyEmploymentStatus(
									sfz.getText().toString(),
									ParseData.getKeyByValue(ParseData.NCJBQK,
											jbqc.getText().toString()),
									ParseData.getKeyByValue(ParseData.JYZT1,
											jyzt.getText().toString()),
									ParseData.getKeyByValue(ParseData.MQJYCY,
											mqjycy.getText().toString()),
									ParseData.getKeyByValue(ParseData.CYHY,
											cyhy.getText().toString()),
									ParseData.getKeyByValue(ParseData.JYXS,
											jyxs.getText().toString()),
									ParseData.getKeyByValue(ParseData.TYSF,
											sffxcyry.getText().toString()),
									ParseData.getKeyByValue(ParseData.JYDQXS,
											jydqlx.getText().toString()),
									ParseData.getKeyByValue(ParseData.JYDQ,
											jydq.getText().toString()),
									ParseData.getKeyByValue(ParseData.JYFS,
											jyfs.getText().toString()), wgnx
											.getText().toString(), jnwgsj
											.getText().toString(), wgygz
											.getText().toString(), ParseData
											.getKeyByValue(ParseData.TYSF,
													sfqdldht.getText()
															.toString()),
									ParseData.getKeyByValue(
											ParseData.SWZYRYJYYX, swzyryjyyx
													.getText().toString()),
									ParseData.getKeyByValue(ParseData.TYSF,
											sfjgbm.getText().toString()),

									JyzkDetail.this.user.getUserid(),
									JyzkDetail.this.user.getOperatorname(),
									JyzkDetail.this.user.getOrg(),
									JyzkDetail.this.user.getZzs051());
						...
						protected void onPostExecute(Boolean result) {
							...
							if (result) {
//								Toast.makeText(JyzkDetail.this, save.getText().toString()+"成功。",
//										Toast.LENGTH_SHORT).show();
								JyzkDetail.this.finish();
							}else {
//								Toast.makeText(JyzkDetail.this, save.getText().toString()+"失败。",
//										Toast.LENGTH_SHORT).show();
							}
						}
						...
						protected void onPreExecute() {
							...
							progressDialog=JyzkDetail.this.commonUtil.	showProgressDialog(save.getText().toString()+"就业状态中...");
		...
		new AsyncTask<Void, Void, Map<String, String>>() {
			...
			protected void onPostExecute(Map<String, String> result) {
				...
				if (result == null) {
					...
					swzyryjyyx.setText(ParseData.SWZYRYJYYX.get(result
							.get("acc905")));
...
//		initAllTextView();
	...
	private String getAllString() {
		return sfz.getText().toString() + "," + name.getText().toString() + ","
				+ jbqc.getText().toString() + "," + jyzt.getText().toString()
				+ "," + mqjycy.getText().toString() + ","
				+ cyhy.getText().toString() + "," + jyxs.getText().toString()
				+ "," + sffxcyry.getText().toString() + ","
				+ jydqlx.getText().toString() + "," + jydq.getText().toString()
				+ "," + jyfs.getText().toString() + ","
				+ jnwgsj.getText().toString() + ","
				+ wgygz.getText().toString() + ","
				+ sfqdldht.getText().toString() + ","
				+ sfjgbm.getText().toString() + ","
				+ swzyryjyyx.getText().toString();

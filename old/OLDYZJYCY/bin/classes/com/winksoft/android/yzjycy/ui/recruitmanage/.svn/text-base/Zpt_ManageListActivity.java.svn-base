 ...
 * Date：2012-10-23
 */
public class Zpt_ManageListActivity extends BaseActivity implements OnScrollListener {
	...
	private void doLoadData() {
		if (pageNum == 0) {
			...
			if(flag==1){//未审核显示图标
				adapter = new CommonAdapter(this, list, R.layout.zpt_manage_list_item, 
	...
	private Runnable myRunnable = new Runnable() {
		public void run() {
			try {
				...
				param.add(new BasicNameValuePair("aab001", user.getUserId()));	//用户ID
	...
	private void addData() {
		...
		if (returnList.get(0).get("state").equals(String.valueOf(Constants.LOGIN_SUCCESS))) {
			...
			for (int i = 0; i < returnList.size(); i++) {
				...
				Bitmap auditStr = BitmapFactory.decodeResource(res, R.drawable.zpt_wsh);
				if ("1".equals(auditTag)) {
					auditStr = BitmapFactory.decodeResource(res, R.drawable.zpt_wsh);
				} else {
					auditStr = BitmapFactory.decodeResource(res, R.drawable.zpt_ysh);
				}
